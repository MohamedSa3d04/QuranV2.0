...
df = pd.read_csv('hafs_smart_v8.csv', low_memory=False, index_col='id')
# print(df[df.index == 6235]['aya_text'].values[0])
def t():
    if 'counter' in st.session_state:
     st.session_state.clear()
sb = st.sidebar
mode = sb.selectbox('حدد ما تريد : ',['قراءة القرءان الكريم', 'الإختبار في القرءان الكريم', 'علّمني ءاية'])
if mode == 'قراءة القرءان الكريم':
    mode = 'reading'
elif mode == 'الإختبار في القرءان الكريم':
    mode = 'testing'
elif mode == 'علّمني ءاية':
    mode = 'teachAya'
...
if mode == 'reading':
    ...
    def clearSS():
        st.session_state.clear()
    modeType = sb.selectbox('النوع : ', ['عشوائي', 'سورة محددة'], on_change=clearSS)
    if modeType == 'عشوائي':
        if 'aya_index' not in st.session_state:
            st.session_state['aya_index'] = np.random.choice(df.index)
    else:
        suraName = sb.selectbox('Enter Sura Name - أدخل اسم السورة:',df['sura_name_ar'].unique(), on_change=clearSS)
        my_df = df[df['sura_name_ar'] == suraName]
        if 'aya_index' not in st.session_state:
            st.session_state['aya_index'] = np.random.choice(my_df.index)

    index = st.session_state['aya_index']
    sura_name = df.loc[index, 'sura_name_ar']
    aya_number = df.loc[index, 'aya_no']
    aya = df.loc[index, 'aya_text']
    tafsir = df.loc[index, 'verse']

    st.markdown(f"<p style='margin : -38px 0px; font-size:50px; font-family : Arabic Typesetting; color:#86EE7C;  direction: rtl;'>اسم السورة : {sura_name}</p>", unsafe_allow_html=True)
    st.markdown(f"<p style='font-size:50px; font-family : Arabic Typesetting; color:#86EE7C;  direction: rtl;'>رقم الآية : {aya_number}</p>", unsafe_allow_html=True)
    st.markdown(f"<p style='margin : -38px 0px; font-size:50px; font-family : Arabic Typesetting; color:red;  direction: rtl;'>الآية : </p>", unsafe_allow_html=True)

    s = f"<p style='font-size:50px; font-family : Arabic Typesetting;  direction: rtl;'>{aya}</p>"
    st.markdown(s, unsafe_allow_html=True)
    st.divider()
    s = f"<p class = 'tafsir' style='font-size:39px; font-family : Arabic Typesetting;  direction: rtl;'>{tafsir}</p>"
    st.markdown(s, unsafe_allow_html=True)

    #Columns
    def next_aya():
        st.session_state['aya_index'] += 1
...
{tafsir}
    ...
    st.divider()
    c1,c2, c3, c4= st.columns((20,20,20,10))
    c2.button('الآية التالية', on_click=next_aya)   
    c3.button('الآية السابقة', on_click=prev_aya)
    c1.button('ءاية أخرى', type='primary', on_click=clearSS)
    c4.button("حفظ الآية", on_click=save_aya)
